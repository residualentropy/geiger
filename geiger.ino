...
const bool LCD_ENABLE = false;
const bool PIEZO_ENABLE = true;

const int PIEZO = 8;
const int HVTRIG = 7;
const int RESP = 2;

const int LED_SENT = A1;
...
void loop() {
  ...
  if (countsNow > countsBeforeLastBeep) {
    if (PIEZO_ENABLE) tone(PIEZO, 740, 10);
  ...
  if ((millis() - cpsTimeBefore) >= 1000) {
    ...
    if (cpsNow > 0) {
      Serial.println(cpsNow);
      tone(LED_SENT, 740, 10);
    }
  }
  ...
  lcd.clear();
  if ((!LCD_ENABLE) && (millis() < 3000)) {
    lcd.setCursor(0, 0);
    lcd.print("LCD DISABLED!");
  }
  if (LCD_ENABLE) {
    lcd.setCursor(0, 0);
    for (int i = 0; i < CPM_INTERVAL_SECS; i++) {
